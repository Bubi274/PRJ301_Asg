...
<head>
    ...
    <title>Tạo thông báo - VITALITY CMS</title>
    ...
    <style>
        ...
        .container { max-width: 600px; margin: 60px auto; padding: 35px; border-radius: 20px;
...
<div class="container">
    <h2><i class="fa-solid fa-bullhorn"></i> Tạo thông báo mới</h2>
    
    <c:if test="${not empty error}">
        <div class="alert"><i class="fa-solid fa-exclamation-circle"></i> ${error}</div>
    </c:if>
    
    <form action="${pageContext.request.contextPath}/manager/notifications/create" method="post">
        <div class="form-group">
            <label for="title"><i class="fa-solid fa-heading"></i> Tiêu đề *</label>
            <input type="text" name="title" id="title" class="form-input" placeholder="Nhập tiêu đề..." value="${title}" required>
        </div>
        <div class="form-group">
            <label for="content"><i class="fa-solid fa-align-left"></i> Nội dung *</label>
            <textarea name="content" id="content" class="form-input" placeholder="Nhập nội dung thông báo..." required>${content}</textarea>
        </div>
        <div class="btn-group">
            <button type="submit" class="btn"><i class="fa-solid fa-paper-plane"></i> Gửi</button>
            <a href="${pageContext.request.contextPath}/manager/notifications" class="btn btn-secondary"><i class="fa-solid fa-xmark"></i> Hủy</a>
        </div>
    </form>
