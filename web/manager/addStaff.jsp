...
<head>
    ...
    <title>Thêm nhân viên - VITALITY CMS</title>
...
<div class="container">
    <h2><i class="fa-solid fa-user-plus"></i> Thêm nhân viên mới</h2>
    
    <c:if test="${not empty error}">
        <div class="alert"><i class="fa-solid fa-exclamation-circle"></i> ${error}</div>
    </c:if>
    
    <form action="${pageContext.request.contextPath}/manager/staffs/add" method="post">
        <div class="form-group">
            <label for="fullName"><i class="fa-solid fa-user"></i> Họ tên *</label>
            <input type="text" name="fullName" id="fullName" class="form-input" placeholder="Nhập họ tên..." value="${fullName}" required>
        </div>
        
        <div class="form-row">
            <div class="form-group">
                <label for="username"><i class="fa-solid fa-at"></i> Tên đăng nhập *</label>
                <input type="text" name="username" id="username" class="form-input" placeholder="5-50 ký tự..." value="${username}" required>
            </div>
            <div class="form-group">
                <label for="email"><i class="fa-solid fa-envelope"></i> Email *</label>
                <input type="email" name="email" id="email" class="form-input" placeholder="email@demo.com..." value="${email}" required>
            </div>
        </div>
        
        <div class="form-group">
            <label for="phone"><i class="fa-solid fa-phone"></i> Số điện thoại</label>
            <input type="tel" name="phone" id="phone" class="form-input" placeholder="Số điện thoại..." value="${phone}">
        </div>
